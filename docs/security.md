...
Queue owners are trusted to choose repositories and local landing branches, providers, per-stage models and instructions, concurrency, structured tasks and dependencies, path policy, approval checkpoints, verification commands, and delivery policy. The same task provider runs both planning and implementation. Queue model names must be valid for the queue's default provider; a task-level provider override uses the override provider's defaults instead of carrying those names across providers. Verification commands execute through the platform shell in temporary task or delivery worktrees. Approval actors and notes are local audit data, not authenticated identities. Do not import queue state from an untrusted source.
...
Run phase, planner output, separate provider session IDs, the complete structured task, dependency result evidence, and the queue workflow captured at claim are persisted. Resuming continues only the retained phase and immutable snapshot in its identity-checked worktree; a resumed planner must still pass cleanliness checks before a fresh implementation process starts. Queue and task edits affect future fresh attempts only. Legacy sessions without a trusted planner handoff are rejected for resume and restart at planning.

## Policy, verification, and approvals

Allowed paths, denied paths, and changed-file limits are enforcement gates, not prompt-only guidance. agentq computes them from Git after implementation and before verified success, checks both sides of renames, and stops a policy-violating task without creating a deliverable result. The same effective policy is evaluated again after a result is replayed onto the integration train. These controls do not make repository paths operating-system read-only while an agent is running; use a stronger sandbox when writes themselves must be impossible.

Queue and task verification commands must all pass, and canonical results record clean-worktree evidence. The durable delivery state distinguishes an implemented branch from verified evidence, a ready immutable result, an integrated train candidate, and a landed local branch. Typed failure classes control whether work retries, replans/rebases, returns to implementation, waits, stops, or requires manual resolution instead of treating every provider exit as equivalent.

Approval checkpoints durably pause before implementation, integration, or landing. An approval permits agentq to continue that configured boundary; it is not cryptographic authorization and does not replace repository access controls or protected remote branches.

## Git behavior

Successful task changes may be committed on dedicated local branches and published under immutable local result refs. Existing or resumed worktrees are identity-checked before use, and cleanup refuses paths outside agentq's managed worktree root. Task and queue deletion is transactionally rejected while any affected task is active or retains a worktree; operators must use the explicit cleanup flow before deleting durable history.

For queues configured with `stack` or `merge-train`, agentq can replay verified commits onto a fenced local train and fast-forward the configured canonical local branch after an operator-confirmed landing or through configured auto-land; any configured checkpoint approval must be durable first. Integration repeats policy and verification in a temporary worktree. Landing refuses a dirty, diverged, non-local, or unexpectedly moved target and uses compare-and-swap state. A checked-out local target may therefore move. agentq does not push remotes, delete the source branch, or open pull requests. Remote review, protected-branch enforcement, and deployment remain outside its trust boundary. `task clean --force --yes` is the explicit destructive worktree cleanup operation.
